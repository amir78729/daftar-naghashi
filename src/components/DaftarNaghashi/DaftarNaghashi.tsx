...
const DrawingComponent = ({
    ...
    const [
        {
            ...
            history,
            mode
        }, dispatch] = useReducer(drawingReducer, {
        isDrawing: false,
        strokeColor: '#000000',
        lineWidth: 5,
        history: [],
    ...
    const saveHistory = () => {
        if (canvasRef.current) {
            dispatch({type: 'SAVE_HISTORY', payload: canvasRef.current.toDataURL()});
        }
    };
    ...
    const handleModeChange = () => {
        dispatch({type: 'SET_MODE', payload: mode === Mode.PEN ? Mode.FILL : Mode.PEN});
    };
    ...
    const handleCanvasClick = (e: MouseEvent<HTMLCanvasElement>) => {
        if (mode === Mode.FILL && canvasRef.current && ctxRef.current) {
            ...
            floodFill(imageData, offsetX, offsetY, targetColor, hexToRgba(strokeColor));
    ...
    return (
        <div>
            ...
            <canvas
                ...
                style={{border: '1px solid black', cursor: mode === Mode.PEN ? 'crosshair' : 'pointer'}}
            />
        </div>
    );
};
